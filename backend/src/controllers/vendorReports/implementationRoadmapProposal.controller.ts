...
const REPORT_TYPE_IMPLEMENTATION_ROADMAP = "Implementation Roadmap Proposal";

/**
 * POST /implementationRoadmapProposal
 * Body: { assessmentId: string, assessmentLabel?: string }
 * Fetches customer risk report + vendor attestation, generates Implementation Roadmap Proposal (sections 28–33), saves to general_reports.
 */
const implementationRoadmapProposal = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const content = await generateImplementationRoadmapProposal(reportJson, attestationSummary);
    ...
    res.status(500).json({
      success: false,
      message: err instanceof Error ? err.message : "Failed to generate Implementation Roadmap Proposal",
    });
  }
};

export default implementationRoadmapProposal;
