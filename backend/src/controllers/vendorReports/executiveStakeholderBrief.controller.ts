...
const REPORT_TYPE_EXECUTIVE_BRIEF = "Executive Stakeholder Brief";

/**
 * POST /executiveStakeholderBrief
 * Body: { assessmentId: string, assessmentLabel?: string }
 * Fetches customer risk report for that assessment + vendor attestation (product) from cots_vendor_assessments,
 * generates Executive Stakeholder Brief (sections 16–21) via LLM, saves to general_reports, returns brief + report row.
 */
const executiveStakeholderBrief = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const brief = await generateExecutiveStakeholderBrief(reportJson, attestationSummary);
    ...
    res.status(500).json({
      success: false,
      message: err instanceof Error ? err.message : "Failed to generate Executive Stakeholder Brief",
    });
  }
};

export default executiveStakeholderBrief;
