...
import { generateComplianceRiskSummaryReport } from "../agents/complianceRiskSummaryReportAgent.js";

const REPORT_TYPE = "Compliance & Risk Summary";
...
/**
 * POST /complianceRiskSummaryReport
 * Body: { assessmentId: string, assessmentLabel?: string }
 */
const complianceRiskSummaryReport = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    try {
      ...
      console.error("complianceRiskSummaryReport getTop5RisksWithMitigations:", e);
    }

    const buyerValidationSnippet = validationSnippetFromCots(cotsRow);

    const crsPayload = await generateComplianceRiskSummaryReport(
    ...
    res.status(200).json({
      success: true,
      data: {
        complianceRiskSummary: crsPayload,
        report: {
    ...
    console.error("complianceRiskSummaryReport error:", err);
    res.status(500).json({
      success: false,
      message: err instanceof Error ? err.message : "Failed to generate Compliance & Risk Summary",
    });
  }
};

export default complianceRiskSummaryReport;
