...
const REQUIRED_KEYS: string[] = []; // e.g. ["attestation", "document_uploads"] if you want strict validation

function validateRequired(formData: Record<string, unknown>): string | null {
 ...
 */
export default async function submit(req: Request, res: Response): Promise<void> {
  try {
    ...
    const err = validateRequired(data as Record<string, unknown>);
    if (err) {
